...
function renderChars(){
  ...
  chars.forEach(c=>{
    ...
    d.innerHTML=`<h3>${c.name}</h3><p>${c.role}</p><p>${locked?'Unlock at player level '+c.unlock:'Unlocked'}</p>`;
    d.onclick=()=>{
      if(locked)return;
...
function unlockedSkillCount(c){return 3}
...
function renderSkills(id){
  ...
  $('#skillContent').innerHTML=`<p id="skillNote">Choose exactly 2 skills for this character. Equipped: ${load.length}/2.</p><div class="skillGrid">${c.skills.map((s,i)=>{let unlocked=i<unlockedSkillCount(c);let eq=load.includes(s[0]);return `<div class="skillCard ${eq?'equipped':''} ${!unlocked?'lockedSkill':''}" onclick="toggleSkill('${c.id}','${s[0]}',${unlocked})"><b>${s[1]}</b> <small>${s[2]}</small><p>${s[3]}</p><small>${unlocked?(eq?'Equipped':'Unlocked - click to equip'):'Locked'}</small></div>`}).join('')}</div>`;
  updateStartButton();
}
...
function resolveMove(){let a=current();let dest=game.path.at(-1);a.x=dest.x;a.y=dest.y;game.path=[];game.phase='action';renderBoard();let usable=getUsableSkills(a);if(usable.length)showActions(a,usable);else{log('No usable equipped skill from final tile. Action skipped.');a.acted=true;game.phase='select';checkTurnDone();renderBoard()}}
...
function useSkill(a,skillId,target){if(['heal','prayer','renew'].includes(skillId))return doSkillHeal(a,target,skillId);if(['guard','shield'].includes(skillId))return doSkillBlock(a,skillId);return doAttack(a,target,skillId)}
function doSkillBlock(a,skillId){$('#actionPanel').classList.add('hidden');setTimeout(()=>{a.block=true;log(a.char.name+' uses '+getSkillDef(a.char.id,skillId)[1]+'.');a.acted=true;afterAction()},350)}
function doSkillHeal(a,target,skillId){let base=skillId==='prayer'?4:skillId==='renew'?2:3;let amt=base+(game.loadouts[a.char.id]?.includes('focus')?1:0);$('#actionPanel').classList.add('hidden');setTimeout(()=>{target.hp=Math.min(target.maxHp,target.hp+amt);log(a.char.name+' uses '+getSkillDef(a.char.id,skillId)[1]+' on '+target.char.name+' for '+amt+'.');a.acted=true;afterAction()},450)}
function doAttack(a,t,skillId='slash'){$('#actionPanel').classList.add('hidden');setTimeout(()=>{let dmg=a.atk+(skillId==='slash'?1:0);if(t.block){dmg=Math.max(0,dmg-2);t.block=false}if(game.loadouts[t.char.id]?.includes('armor'))dmg=Math.max(0,dmg-1);t.hp=Math.max(0,t.hp-dmg);let name=(getSkillDef(a.char.id,skillId)?.[1]||'Attack');log(a.char.name+' uses '+name+' on '+t.char.name+' for '+dmg+'.');a.acted=true;afterAction()},450)}
